...
static void inference_and_performance(int deviceid, const string& engine_file, TRT::Mode mode, Yolo::Type type,
    ...
    for (int i = 0; i < 10; ++i) boxes_array = engine->commits(images);
    ...
    for (int i = 0; i < ntest; ++i) boxes_array = engine->commits(images);
...
void multi_gpu_test() {
    ...
    auto multi_gpu_infer = Yolo::create_multi_gpu_infer("yolov5s-6.0.FP32.trtmodel", Yolo::Type::V5, devices);
